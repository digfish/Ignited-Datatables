 ...
 * Changed by Samuel Viana in order to allow transformation of the data return from the the data prior to serialize in JSON
 *
 * @package    CodeIgniter
 ...
 *              Samuel Viana <pescadordigital@gmail.com>
...
class Datatables {
     ...
     * changed by sdviana allowing to pass a callback that will transform the raw data return by the query
     * this callback needs one argument variable that identifies the data return from the query
     * @param string charset
     * @return string
     */
    public function generate($charset = 'UTF-8', $filterdata_callback = NULL) {
        $this->get_paging();
        $this->get_ordering();
        $this->get_filtering();
        return $this->produce_output($charset, $filterdata_callback);
    }
     ...
     * @return string
     */
    protected function produce_output($charset, $filter_callback = NULL) {
        $aaData = array();
        ...
        if (!empty($filter_callback)) {
            $aaData = call_user_func($filter_callback, $aaData);
        };
